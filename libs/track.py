...
class Track:
    ...
    The data representation is:
        ...
        - segments: each component of the track, each gpx file is a segment
    ...
    def to_json(self) -> str:
        """
        Construct a json string with all the needed track object contents to
    ...
    def from_json(cls, json_file: str) -> Track:
        """
        Construct  track from a json_file previously generated by the
        .to_json method
        :param json_file: path to input file
    ...
    def get_segment(self, index: int) -> pd.DataFrame:
        """
        Filter the dataframe to extract the desired index
        :param index: index to the segment
        :return: segment pandas dataframe
        """
        return self.df_track[self.df_track['segment'] == index]

    def reverse_segment(self, index: int):
        """
        Reverse the sub-dataframe for the desired index
        :param index: index to the segment
        :return: None
        """
    ...
    @staticmethod
    def _get_speed_factor_to_slope(slope: float) -> float:
        """
    ...
    def insert_timestamp(self, initial_time: dt.datetime,
        ...
        """
        Insert time column in the track dataframe. Constant speed is a
        priori supposed, but it can be adapted to elevation changes.
        :param initial_time: initial timestamp of the track
        :param desired_speed: average speed
        :param consider_elevation: adapt speed to elevation changes
        :return: None
        """
    ...
    def smooth_elevation(self, index: int):
        """
        Apply moving average algorithm to the elevation of a given segment
        :param index: index to the used segment
        :return: None
        """
    ...
    def fix_elevation(self, index: int):
        """
        Detect and remove any kind of cliff in the middle of the track
        :param index: index to the fixed segment
        :return: None
        """
        df_segment = self.get_segment(index)
    ...
    def remove_segment(self, index: int):
        """
        Remove one segment from the track dataframe
        :param index: index to the fixed segment
        :return: size of segments after removal
        """
    ...
    def change_order(self, new_order: dict):
        ...
        :param new_order: dictionary with keys-former and values-new.
        Example {1: 3, 2: 1, 3: 2}
        :return: None
        """
    ...
    def rename_segment(self, index: int, new_name: str) -> bool:
        ...
        :param index: index to the fixed segment
        :param new_name: string with new segment name
        :return: True if the change success
        """
        try:
    ...
    def update_summary(self):
        """
        Update all the metadata which described the track characteristics
        :return: None
        """
    ...
    def get_summary(self):
        """
        Construct a dictionary with all the metadata describing the segments
    ...
    def _force_columns_type(self):
        """
        Force the column of the track dataframe to have the expected type
        :return: None
        """
    ...
    def _insert_positive_elevation(self):
        """
        Add new column to track dataframe, containing the cumulative positive
        gained elevation.
        :return: None
        """
    ...
    def _insert_negative_elevation(self):
        """
        Add new column to track dataframe, containing the cumulative negative
        lost elevation.
        :return: None
        """
    ...
    def _insert_distance(self):
        """
        Add new column to track dataframe, containing the cumulative distance
        :return: None
        """
    ...
    def _update_extremes(self):
        """
        Update the extreme coordinates most and lowest latitude/longitude
        :return: None
        """
        self.extremes = \
...
class SummaryUtils:
    ...
    def get_distance_label(ob_track: Track, segment_id: int = 1,
        ...
        """
        Construct a label with proper units and decimal places for total
        distance of segment of whole track
        :param ob_track: track object
        :param segment_id: segment index to use
        :param total: True to use whole track, not single segment
    ...
    def get_elevation_label(ob_track: Track, magnitude: str,
        ...
        distance of segment of whole track
        :param ob_track: track object
        :param magnitude: assign pos/neg to produce positive or negative
        elevation labels
        :param segment_id: segment index to use
        :param total: True to use whole track, not single segment
        :return: distance label
